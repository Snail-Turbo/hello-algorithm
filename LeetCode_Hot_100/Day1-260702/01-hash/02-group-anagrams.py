...
def group_anagrams_upgrade(strs):
    ...
    for s in strs:
        ...
        # bytearray(26) 比 [0]*26 更紧凑（每个元素占 1 字节 vs 28 字节）
        counts = bytearray(26)
        for ch in s:
            # 关键点：ord() 函数返回字符的 Unicode 码点，ord('a') 返回 97，ord('b') 返回 98，以此类推。
            counts[ord(ch) - ord('a')] += 1

        # bytes(counts) 是不可变字节序列，哈希快，作为字典键比 tuple / str 更高效
        mp.setdefault(bytes(counts), []).append(s)
